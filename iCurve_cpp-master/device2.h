...
// (string model, string tool, int mats, float node, float max_move, float max3, float curve_tol, vector<material> temp) {
		...
		for (int i = 0; i < temp.size(); i++) {
			...
			cout << "device length: " <<device_length << endl;
		}
	}
	...
	// float get_max_advance() {return max_advance;}

	// float get_max3d() {return max_3d;}
